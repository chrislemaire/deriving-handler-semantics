...
module Final.Writing.VeselySimpleTest (tests) where

import Control.Monad.State
import Data.Maybe (maybeToList)
import Final.TestUtil
import Final.Writing.VeselySimple
...
tests =
  testGroup
    "Simple Vesely - One step at a time language"
    [ testGroup
        "Generated"
        [ testProperty "Equivalence of normalise0 and normalise1" $
          ...
          testProperty "Equivalence of normalise0 and normalise8" $
            withMaxSuccess 10000 $
              programRunsTheSameProperty id normalise0 normalise8 (resize 1000 arbitraryExpr),
          testProperty "Equivalence of normalise0 and normalise9" $
            withMaxSuccess 10000 $
              programRunsTheSameProperty id normalise0 normalise9 (resize 1000 arbitraryExpr)
        ]
    ]
